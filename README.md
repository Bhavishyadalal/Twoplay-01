...
![divider](https://capsule-render.vercel.app/api?type=rect&color=0:FF6FA5,50:A06CFF,100:6C8BFF&height=3&section=header)
...
<svg width="110" height="110" viewBox="0 0 42 42" xmlns="http://www.w3.org/2000/svg">
  <style>
    .cat-tail{animation:sway 2.4s ease-in-out infinite;transform-origin:31px 30px;}
    .cat-ear-l{animation:twitchL 3.2s ease-in-out infinite;transform-origin:10px 12px;}
    .cat-ear-r{animation:twitchR 3.6s ease-in-out infinite;transform-origin:32px 12px;}
    .cat-eye{animation:blink 3.8s ease-in-out infinite;transform-origin:center;}
    @keyframes sway{0%,100%{transform:rotate(0deg);}50%{transform:rotate(-8deg);}}
    @keyframes twitchL{0%,90%,100%{transform:rotate(0deg);}92%{transform:rotate(-6deg);}96%{transform:rotate(4deg);}}
    @keyframes twitchR{0%,88%,100%{transform:rotate(0deg);}91%{transform:rotate(6deg);}95%{transform:rotate(-4deg);}}
    @keyframes blink{0%,92%,100%{transform:scaleY(1);}95%{transform:scaleY(0.1);}}
  </style>
  <path class="cat-tail" d="M31 30c4 1 6-2 5-5" stroke="#FFD9EC" stroke-width="2.6" stroke-linecap="round" fill="none"/>
  <path class="cat-ear-l" d="M12 15 9 6l7 4Z" fill="#FFE3F0" stroke="#2B2440" stroke-width="1"/>
  <path class="cat-ear-r" d="M30 15l3-9-7 4Z" fill="#FFE3F0" stroke="#2B2440" stroke-width="1"/>
  <ellipse cx="21" cy="22" rx="12" ry="10.5" fill="#FFF3E4" stroke="#2B2440" stroke-width="1"/>
  <rect class="cat-eye" x="14.5" y="20" width="3.4" height="4.2" rx="1.7" fill="#2B2440"/>
  <rect class="cat-eye" x="24.1" y="20" width="3.4" height="4.2" rx="1.7" fill="#2B2440"/>
...
That's the header‑bar mascot, redrawn here with its actual idle motion — the tail sways, the ears twitch, and it blinks on a loop, exactly like it does live in the app. Tap the real one in‑app and it winks; leave it alone and it occasionally strikes a hidden pose (hearts, flowers) before settling back down. Every animation on this page, including this one, is real SVG/CSS — nothing here is a video or a GIF pretending to move.
...
![divider](https://capsule-render.vercel.app/api?type=rect&color=0:6C8BFF,50:2FBF83,100:FFB43D&height=3&section=header)
...
<details>
<summary><b>🐎 See the chess pick card animate on hover</b> — click to expand</summary>

<br>

<svg width="100%" height="90" viewBox="0 0 400 90" xmlns="http://www.w3.org/2000/svg">
  <style>
    .pick-card{transition:transform .18s;}
    .pick-group:hover .pick-card{transform:translateY(-4px) rotate(-1deg);}
    .pick-group:hover .pick-icon{transform:scale(1.15) rotate(4deg);}
    .pick-icon{transition:transform .18s;transform-origin:center;}
  </style>
  <g class="pick-group">
    <rect class="pick-card" x="10" y="10" width="110" height="70" rx="16" fill="#FFF3E4" stroke="#A06CFF" stroke-width="2"/>
    <text class="pick-icon" x="65" y="48" font-size="28" text-anchor="middle">♞</text>
    <text x="65" y="68" font-size="11" font-weight="800" text-anchor="middle" fill="#2B2440" font-family="sans-serif">Chess</text>
  </g>
  <text x="150" y="48" font-size="13" fill="#6B6180" font-family="sans-serif">↖ hover the card (on GitHub this renders live — lift + icon spin)</text>
</svg>
...
![divider](https://capsule-render.vercel.app/api?type=rect&color=0:FF5C7A,50:FF9EBE,100:FF6FA5&height=3&section=header)
...
![divider](https://capsule-render.vercel.app/api?type=rect&color=0:FFB43D,50:2FBF83,100:6C8BFF&height=3&section=header)
...
<svg width="100%" viewBox="0 0 640 190" xmlns="http://www.w3.org/2000/svg" font-family="'JetBrains Mono', monospace">
  <style>
    .pulse{animation:pulse 1.8s ease-in-out infinite;}
    .pulse2{animation:pulse 1.8s ease-in-out infinite .5s;}
    .flow{stroke-dasharray:6 6;animation:dash 1.2s linear infinite;}
    @keyframes pulse{0%,100%{opacity:.35;}50%{opacity:1;}}
    @keyframes dash{to{stroke-dashoffset:-24;}}
  </style>
  <circle class="pulse" cx="90" cy="95" r="34" fill="#FFE3F0" stroke="#FF6FA5" stroke-width="2"/>
  <text x="90" y="90" font-size="24" text-anchor="middle">🧑</text>
  <text x="90" y="145" font-size="12" text-anchor="middle" fill="#2B2440">You</text>
  <text x="90" y="160" font-size="10" text-anchor="middle" fill="#6B6180">code: 12345</text>

  <circle class="pulse2" cx="550" cy="95" r="34" fill="#E3E9FF" stroke="#6C8BFF" stroke-width="2"/>
  <text x="550" y="90" font-size="24" text-anchor="middle">🧑</text>
  <text x="550" y="145" font-size="12" text-anchor="middle" fill="#2B2440">Friend</text>
  <text x="550" y="160" font-size="10" text-anchor="middle" fill="#6B6180">enters 12345</text>

  <line class="flow" x1="128" y1="95" x2="512" y2="95" stroke="#A06CFF" stroke-width="3"/>
  <text x="320" y="80" font-size="12" text-anchor="middle" fill="#A06CFF" font-weight="700">direct P2P data channel</text>
  <text x="320" y="115" font-size="10" text-anchor="middle" fill="#6B6180">moves · chat · draws · voice</text>
</svg>
...
![divider](https://capsule-render.vercel.app/api?type=rect&color=0:2FBF83,50:6C8BFF,100:A06CFF&height=3&section=header)
...
![divider](https://capsule-render.vercel.app/api?type=rect&color=0:A06CFF,50:FF6FA5,100:FFB43D&height=3&section=header)
...
One file, on purpose — trivially portable, at the cost of some scrolling. The `GameControllers` registry near the bottom of the `<script>` is the map: search `GameControllers.<name>` to find or add a game.
